...
async function excluirServico(id) {
  ...
  try {
    ...
    async function atualizarStatus(id, novoStatus) {
        ...
        try {
            const response = await fetch(`/admin/agendamentos/status/${id}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async function verDetalhes(id) {
        ...
        try {
            const response = await fetch(`/admin/agendamentos/detalhes/${id}`);
            const data = await response.json();
            ...
            if (data.sucesso) {
                ...
                conteudo.innerHTML = `
                    ...
                    <div class="detalhes-section">
                        <h4>📋 Serviços</h4>
                        ${ag.servicos.map(s => `<p>• ${s.Nome} - R$ ${s.Valor.toFixed(2).replace('.', ',')}</p>`).join('')}
                        <p><strong>Total: R$ ${ag.valorTotal.toFixed(2).replace('.', ',')}</strong></p>
                    </div>
                    ${ag.observacoes ? `
                    <div class="detalhes-section">
                        <h4>📝 Observações</h4>
